/*
 * @Description:
 * @Version:
 * @Autor: mzc
 * @Date: 2022-08-20 14:24:47
 * @LastEditors: mzc
 * @LastEditTime: 2023-03-05 23:11:13
 */
export const LOGIN = "login";

export const HOME = "home";

export const DOWNLOAD = "download";

export const MAIN = "main";
export const MAIN_RESOURCE = "main-resource";
export const MAIN_RESOURCE_MYRESOURCE = "main-resource-myResource";
export const MAIN_RESOURCE_RESOURCEITEM = "main-resources-resourceItem";
export const MAIN_RESOURCE_FOLDERITEM = "main-resources-folderItem";

export const MAIN_BACKLOG = "main-backlog";
export const MAIN_BACKLOG_MY_BACKLOG = "main-backlog-my-backlog";

export const MAIN_CHATTING = "main-chatting";

export const MAIN_COURSE = "main-course";
export const MAIN_COURSE_LISTS = "main-course-lists";
export const MAIN_COURSE_ITEM = "main-course-item";
export const MAIN_COURSE_ITEM_CONTENT = "main-course-item-content";
export const MAIN_COURSE_ITEM_COMMENTS = "main-course-item-comments";
export const MAIN_COURSE_ITEM_TITLE_DETAIL = "main-course-item-title-detail";
export const MAIN_COURSE_TASKS = "main-course-tasks";

export const MAIN_STUDYROOM = "main-studyRoom";
export const MAIN_STUDYROOM_HOME = "main-studyRoom-home";
export const MAIN_STUDYROOM_DETAIL = "main-studyRoom-detail";

export const MAIN_RESOURCESQUARE = "main-resourceSquare";
export const MAIN_RESOURCESQUARE_HOME = "main-resource-square-home";
export const MAIN_RESOURCESQUARE_RANKS = "main-resources-square-ranks";
export const MAIN_RESOURCESQUARE_ITEM_DETAIL = "main-resource-item-detail";
export const MAIN_RESOUCESQUARE_SEARCH = "main-resource-search";

export const MAIN_COMMUNITY = "main-community";
export const MAIN_COMMUNITY_HOME = "main-community-home";
export const MAIN_COMMUNTIY_LIKES = "main-community-likes";
export const MAIN_COMMUNITY_COLLECTIONS = "main-community-collections";
export const MAIN_COMMUNITY_ARTICLE_ADD = "main-community-article-add";
export const MAIN_COMMUNITY_ARTICLE_DETAIL = "main-community-article-detail";

export const MAIN_MY_CENTER = "main-my-center";
